...
Koopa::Koopa(float x, float y, int scale, Handler *handler, UI *ui, sf::RenderWindow &window)
    : Enemy(x, y, scale, handler, ui, window, EnemyCharacter::Koopa)
{
    if (handler == nullptr)
    {
    ...
    std::vector<sf::Texture> normalKoopaTextures;
    for (int i = 0; i < 2; ++i)
    {
        normalKoopaTextures.push_back(koopaTextures[i]);
    }
    normalKoopa = new Animation(2, normalKoopaTextures, window);

    std::vector<sf::Texture> shellKoopaTextures;
    for (int i = 4; i < 6; ++i)
    {
        shellKoopaTextures.push_back(koopaTextures[i]);
    }
    shellKoopa = new Animation(2, shellKoopaTextures, window);
...
void Koopa::render()
{
    ...
    else if (state == KoopaState::Shell)
    {
        ...
        {
            sf::Sprite sprite;
            sprite.setTexture(koopaTextures[4]);
            sprite.setPosition(getX(), getY());
            float scaleX = getWidth() / koopaTextures[4].getSize().x;
            float scaleY = getHeight() / koopaTextures[4].getSize().y;
            sprite.setScale(scaleX, scaleY); // Set the scale of the sprite

            getWindow().draw(sprite); // Draw the shell texture
        }
        else // With legs
            currentAnimation->drawAnimation(getX(), getY(), (float)getWidth(), (float)getHeight());
    }
    else if (state == KoopaState::ShellMoving) // No legs
    {
        ...
        float scaleY = getHeight() / koopaTextures[4].getSize().y;
        sprite.setScale(scaleX, scaleY); // Set the scale of the sprite
        getWindow().draw(sprite);        // Draw the shell moving texture
    }
    else if (state == KoopaState::DeadByFire)
    {
        ...
        float scaleX = getWidth() / koopaTextures[4].getSize().x;
        float scaleY = getHeight() / koopaTextures[4].getSize().y;
        sprite.setOrigin(0, koopaTextures[4].getSize().y); // Set the origin to the bottom-left corner
        sprite.setScale(scaleX, -scaleY);                  // Set the scale of the sprite
        getWindow().draw(sprite);                          // Draw the dead Koopa texture
    }
}
...
sf::FloatRect Koopa::getBoundsTop()
{
    if (state == KoopaState::Shell)
    {
...
sf::FloatRect Koopa::getBoundsRight()
{
    if (state == KoopaState::Shell)
    {
...
sf::FloatRect Koopa::getBoundsLeft()
{
    if (state == KoopaState::Shell)
    {
...
void Koopa::setStomped(bool newState)
{
    if (newState)
    {
        ...
        sprite.setTexture(koopaTextures[4]);
        sprite.setPosition(getX(), getY());
        sprite.setScale(getWidth(), getHeight()); // Set the scale of the sprite
        getWindow().draw(sprite);                 // Draw the stomped Koopa texture
    }
...
Koopa::~Koopa()
{
    if (normalKoopa == shellKoopa)
    {
        // Both point to the same Animation, delete once
        delete normalKoopa;
    }
    else
    {
        // Delete both if they are different
        delete normalKoopa;
        delete shellKoopa;
    }
    normalKoopa = nullptr;
