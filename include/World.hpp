...
public:
    ...
    void loadFromJSON(const json& j)
    {
        ...
        {
            Entity* entity = createEntity(stoi(id));
        }
        ...
        for (const auto& [id, data] : j.items())
        {
            for (const auto& [name, compJSON] : data.items())
            {
                ...
                loadByType[name](compJSON, World::getEntityByID(stoi(id)));
            }
        }
    }
...
public:
    Entity* getEntityByID(int id)
    {
        ...
        // throw std::runtime_error("Cannot find entity with id" + std::to_string(id));
        return nullptr;
    }
};